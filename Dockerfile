...
RUN conda install -n anaconda11 pytorch==2.2.0 torchvision==0.17.0 torchaudio==2.2.0 -c pytorch -c nvidia -y
...
RUN conda install -n tf conda-forge::tensorflow -y
RUN conda run -n tf python -m ipykernel install --user --name=tf --display-name="Tensorflow (python 3.11)"
